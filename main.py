    ...
    class App(customtkinter.CTk):
        def __init__(self):
            ...
            def selecionar_arquivo():
                arquivo_pdf = customtkinter.filedialog.askopenfile(
                ...
                if arquivo_pdf:
                    ...
                    try:
                        shutil.copy(
                            arquivo_pdf.name, caminho_destino
                        )  # Certifique-se de usar o 'name' do arquivo
            ...
            self.configure(
                fg_color=my_custom_theme["primary_color"]
            )  # Light Grayish Blue

            linhas_frame = customtkinter.CTkFrame(
                self, fg_color=None, width=largura, height=75
            )
            ...
            canvas_1.create_rectangle(
                0, 0, largura, 25, outline="", width=3, fill="white"
            )
            # Draw the second rectangle with color2
            canvas_1.create_rectangle(
                0, 25, largura, 75, outline="", width=3, fill=my_custom_theme["color2"]
            )
            # Draw the third rectangle with color3
            canvas_1.create_rectangle(
                0, 75, largura, 100, outline="", width=3, fill=my_custom_theme["color8"]
            )
            ...
            canvas_2.create_rectangle(
                0, 0, largura - 50, 25, outline="", width=3, fill="white"
            )
            canvas_2.create_rectangle(
                0, 30, largura - 50, 150, outline="", width=3, fill="white"
            )
            ...
            welcome_frame = customtkinter.CTkFrame(
                self, fg_color="white", width=largura - 40, height=150
            )
            welcome_frame.place(x=largura / 2.7, y=550)

            welcome_text_1 = customtkinter.CTkLabel(
                welcome_frame,
            ...
            welcome_text_2 = customtkinter.CTkLabel(
                welcome_frame,
                text="Calculadora de Saldo de Liquidação de Portabilidades",
                text_color="#5bc2e7",
            ...
            botao = customtkinter.CTkButton(
                self,
                text="Calcular Saldo de Liquidação",
                width=50,
                height=20,
                fg_color="white",
                text_color="black",
            ...
            # Página 2
            entrada_frame = customtkinter.CTkFrame(
                self, fg_color="white", width=largura - 40, height=500
            )
            entrada_label = customtkinter.CTkLabel(
                entrada_frame,
                text="Por favor, insira o CPF do Cliente",
                text_color="black",
                font=("Times New Roman", 16),
            )
            entrada = customtkinter.CTkEntry(
                entrada_frame, 250, 25, None, None, bg_color="white", text_color="black"
            )
            CPF_extraido = customtkinter.CTkLabel(entrada_frame)
            entrada_botao = customtkinter.CTkButton(
                entrada_frame,
                width=50,
                height=20,
                text="Gravar CPF",
                fg_color=my_custom_theme["color10"],
                text_color="black",
                command=ocultar_2,
            )

            # página 3
            anexo_frame = customtkinter.CTkFrame(
                self, fg_color="white", width=largura - 40, height=500
            )
            anexo_label = customtkinter.CTkLabel(
                anexo_frame,
                text="O Extrato de Empréstimos deve estar em .pdf e ser criado digitalmente",
                text_color="black",
                font=("Times New Roman", 16),
            )
            anexo_botao = customtkinter.CTkButton(
                anexo_frame,
                width=50,
                height=20,
                text="Anexar arquivo",
        ...
        for page in pdf.pages:
            ...
            for table in page.extract_tables():
                ...
                for linha in table[2:]:
                    ...
                    if (
                        len(linha) > 13 and linha[2] == "Ativo"
                    ):  # Coluna de situação (index 2)
                        ...
                        taxa = np.round(
                            calcular_taxa(parcelas, valor_parcela, valor_emprestado)
                            * 100,
                            2,
                        )

                        valor_emprestado, taxa = verificar_dados(
                            parcelas, valor_parcela, valor_emprestado, taxa, contrato
                        )

                        # Calcular o valor de liquidação
                        valor_liquidacao = np.round(
                            calcular_liquidacao(valor_parcela, meses_restantes, taxa), 2
                        )
                        print(f"{taxa}")
...
def calcular_taxa(n_periodos, pmt, valor_presente, chute_inicial=0.01):
    ...
    try:
        taxa = fsolve(funcao_taxa, chute_inicial, maxfev=1000)[
            0
        ]  # Aumentar maxfev se necessário
    ...
    if valor_emprestado <= 0 or taxa <= 1.3:
        ...
        while valor_emprestado <= 0 or taxa <= 1.3:
            try:
                valor_emprestado = float(
                    input(
                        f"O contrato {contrato} está com valor emprestado incorreto para o cáculo, por favor informe um valor emprestado válido (VALOR LIBERADO): "
                    )
                )
                taxa = np.round(
                    calcular_taxa(parcelas, valor_parcela, valor_emprestado) * 100, 2
                )
...
table.auto_set_column_width(
    [0, 1, 2, 3, 4, 5, 6, 7, 8]
)  # Ajusta automaticamente todas as colunas
